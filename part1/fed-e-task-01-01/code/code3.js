...
// 2.实现 ex1 函数
let ex1 = x => {
  return fp.map(item => fp.add(1, item), x)
}
// 3.调用测试
console.log(maybe.map(ex1)) // Maybe { _value: [ 6, 7, 2 ] }

// console.log('nima', [5, 6, 1].map(item => item + 1))
/*
  练习2:
  实现 ex2 函数
...
// 1.实现 ex3
let ex3 = () => {
  let data = safeProp('name', user)
  return data.map(fp.first)
}
// 2.测试打印
...
// 1.实现 ex4 函数
let ex4 = (x) => {
  return Maybe.of(x).map(parseInt)
